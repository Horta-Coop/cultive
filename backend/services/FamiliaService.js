...
export const FamiliaService = {
  getAllFamilias: async ({ requester, page, limit } = {}) => {
    ...
    switch (requester.role) {
      ...
      case "gestor":
        return await FamiliaRepository.findByGestorId(requester.id, { take, skip });
  ...
  getFamiliaById: async (id, requester) => {
    ...
    if (["cultivador", "voluntario"].includes(requester.role)) {
      if (requester.familiaId && requester.familiaId === familia.id) return familia;
      throw new Error("Acesso negado");
    }
  ...
  updateFamilia: async ({ id, data, requester }) => {
    ...
    if (requester.role === "admin") return await FamiliaRepository.updateFamilia(id, data);
    if (requester.role === "gestor") {
