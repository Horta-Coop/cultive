...
const DashboardSidebar = ({ user, isOpen, onToggle }) => {
  ...
  const menuItems = [
    ...
    /* { path: '/diversificacao', icon: PieChart, label: 'Diversificação' }, */
    /* { path: '/planejamento', icon: Calendar, label: 'Planejamento' }, */
    /* { path: '/distribuicao', icon: Package, label: 'Distribuição' }, */
    { path: '/comunicacao', icon: MessageSquare, label: 'Comunicação' },
    ...(user.role === 'admin' ? [{ path: '/usuarios', icon: Users, label: 'Usuários' }] : []),
    ...(user.role === 'gestor' || user.role === 'admin' ? [{ path: '/familias', icon: Users, label: 'Famílias' }]:[]),
      ...
      <div className="flex flex-col bg-base-100 border-r border-base-300 h-full relative transition-all duration-300 ease-in-out w-64">
        ...
        <div className="p-4 border-b border-base-300 flex items-center justify-between">
          ...
          >
            ...
            <span className="ml-2 text-xl font-semibold text-base-content">Cultive</span>
          </div>
          <button 
